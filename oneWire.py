...
def checkSlaves():
    ...
    if slaveCount == "0":
        logger.debug('Slaves not detected by kernel')
...
class OneWire:
    def __init__(self, address,
                 gpio=19):  # use gpio 19 by default if not specified
        """
    ...
    def __prepare(self):
        ...
        if not setupOneWire(self.gpio):
            self.logger.warning('Could not set up 1-Wire on GPIO {}'.format(
                self.gpio))
            return False

        # check if the kernel is recognizing slaves
        if not checkSlaves():
            self.logger.warning('Kernel is not recognizing slaves.')
            return False
        ...
        if not checkRegistered(self.address):
            ...
            self.logger.warning('Device is not registered on the bus.')
            return False
